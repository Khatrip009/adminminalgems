...
interface CustomerFormState {
  id?: string;
  email: string;
  name: string;         // maps to backend 'name' field
  phone: string;
  company: string;
  country: string;
...
const calculateAnalytics = (orders: CustomerOrder[]): CustomerAnalytics => {
  ...
  const totalItems = orders.reduce((sum, o) => sum + (o.items_count || 0), 0);
  const ltv = orders.reduce((sum, o) => sum + Number(o.grand_total || 0), 0);
...
const CustomerPage: React.FC<CustomerPageProps> = ({ customerId, onClose }) => {
  ...
  const [detailCurrentOrder, setDetailCurrentOrder] = useState<CustomerOrder | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        // Use `customer.name` (from detail endpoint) or fallback to `(customer as any).full_name`
        const customerName = customer.name || (customer as any).full_name || "";
        setForm({
          id: customer.id,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Payload uses 'name' because backend expects that field
  ...
  const analytics = useMemo(() => calculateAnalytics(detailOrders), [detailOrders]);
      ...
      <div className="mb-5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-xl font-semibold text-slate-900 dark:text-white">
              {form.name || detailCustomer?.name || "Customer details"}
            </h2>
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Edit profile, review orders, analytics and manage addresses for this customer.
            </p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="inline-flex items-center gap-2 rounded-full border border-slate-300 bg-white px-3 py-1 text-sm hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200"
          >
            ...
            <form onSubmit={handleSubmit} className="space-y-5 text-sm">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-2 pt-6">
                  ...
                  />
                  <label htmlFor="cust_phone_verified" className="text-xs text-slate-700 dark:text-slate-200">
                    Phone verified
                  </label>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-3">
                <p className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                  <Ban size={12} />
                  Blocked status can be managed via metadata.
                </p>
                <div className="flex items-center gap-3">
                  ...
                  >
                    {saving ? <Loader2 className="animate-spin" size={14} /> : <><Edit2 size={14} /> Save Changes</>}
                  </button>
                </div>
              </div>
            </form>
            ...
            <div className="space-y-5 text-sm">
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/40">
                <div className="mb-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">Current order</h3>
                  </div>
                  {detailCurrentOrder?.id && <span className="text-xs text-slate-500">Order ID: {detailCurrentOrder.id}</span>}
                </div>
                {detailCurrentOrder ? (
                  <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
                    <div className="space-y-1">
                      <div className="text-xs text-slate-500">Status overview</div>
                      <div className="flex flex-wrap gap-1">
                        <span className={"inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-medium " + getStatusBadgeClass(detailCurrentOrder.status)}>
                          <Package size={11} />{detailCurrentOrder.status}
                        </span>
                        <span className={"inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-medium " + getStatusBadgeClass(detailCurrentOrder.payment_status || "")}>
                          <CreditCard size={11} />{detailCurrentOrder.payment_status || "—"}
                        </span>
                        <span className={"inline-flex items-center gap-1 rounded-full border px-2 py-1 text-[11px] font-medium " + getStatusBadgeClass(detailCurrentOrder.fulfillment_status || "")}>
                          <Truck size={11} />{detailCurrentOrder.fulfillment_status || "—"}
                        </span>
                      </div>
                    </div>
                    <div className="space-y-1">
                      <div className="text-xs text-slate-500">Totals</div>
                      <div className="text-base font-semibold">{formatMoney(detailCurrentOrder.grand_total)}</div>
                      <div className="text-[11px] text-slate-500">Items: {detailCurrentOrder.items_count} · Subtotal: {formatMoney(detailCurrentOrder.subtotal)} · Shipping: {formatMoney(detailCurrentOrder.shipping_total)}</div>
                    </div>
                    <div className="space-y-1">
                      <div className="text-xs text-slate-500">Shipping / timeline</div>
                      <div className="space-y-0.5 text-[11px] text-slate-500">
                        <div>Placed: {formatDateTime(detailCurrentOrder.placed_at)}</div>
                        <div>Shipped: {formatDateTime(detailCurrentOrder.shipped_at)}</div>
                        <div>Completed: {formatDateTime(detailCurrentOrder.completed_at)}</div>
                        {detailCurrentOrder.shipments?.length > 0 && detailCurrentOrder.shipments[0].tracking_number && (
                          <div>Tracking: {detailCurrentOrder.shipments[0].tracking_number}</div>
                        )}
                      </div>
                    </div>
                  </div>
                ) : (
                  <div className="text-xs text-slate-500">No active/current order found for this customer.</div>
                )}
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-900">
                <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 text-sm font-medium text-slate-700 dark:border-slate-700 dark:text-slate-200">
                  ...
                  <span className="text-xs text-slate-500">Page {detailOrdersPage} of {detailOrdersTotalPages}</span>
                </div>
                {detailOrders.length === 0 ? (
                  <div className="px-4 py-6 text-sm text-slate-500">No orders found for this customer.</div>
                ) : (
                      ...
                      <div key={o.id} className="px-4 py-3 text-sm">
                        <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-[11px] text-slate-500">{formatDateTime(o.placed_at)}</span>
                              </div>
                              <div className="mt-1 flex flex-wrap gap-1">
                                <span className={"inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium " + getStatusBadgeClass(o.status)}>
                                  <Package size={11} />{o.status}
                                </span>
                                <span className={"inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium " + getStatusBadgeClass(o.payment_status || "")}>
                                  <CreditCard size={11} />{o.payment_status || "—"}
                                </span>
                                <span className={"inline-flex items-center gap-1 rounded-full border px-2 py-0.5 text-[11px] font-medium " + getStatusBadgeClass(o.fulfillment_status || "")}>
                                  <Truck size={11} />{o.fulfillment_status || "—"}
                                </span>
                              </div>
                              <div className="mt-1 text-[11px] text-slate-500">
                                Total: <span className="font-semibold">{formatMoney(o.grand_total)}</span> · Items: {o.items_count}
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="mt-2 grid flex-1 grid-cols-1 gap-2 md:mt-0 md:grid-cols-2">
                              ...
                              {o.items?.length ? (
                                <ul className="space-y-0.5">
                                  {o.items.slice(0, 3).map((it) => (
                                    <li key={it.id}>{it.title || "Item"} × {it.quantity} — {formatMoney(it.line_total)}</li>
                                  ))}
                            ...
                            <div className="text-xs text-slate-500 dark:text-slate-400">
                              <div className="mb-1 font-semibold">Shipments</div>
                              {o.shipments?.length ? (
                                <ul className="space-y-0.5">
                                  {o.shipments.map((s) => (
                                    <li key={s.id}>
                                      <span className="inline-flex items-center gap-1">
                                        <Truck size={11} /><span>{s.status}</span>
                                      </span>
                ...
                <div className="flex items-center justify-between border-t border-slate-200 px-4 py-2 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-400">
                  <div>Page {detailOrdersPage} of {detailOrdersTotalPages}</div>
                  <div className="space-x-2">
                    <button
                      disabled={detailOrdersPage <= 1 || modalLoading}
                      onClick={() => { const next = detailOrdersPage - 1; if (next < 1) return; loadOrdersPageForCustomer(next); }}
                    ...
                    <button
                      disabled={detailOrdersPage >= detailOrdersTotalPages || modalLoading}
                      onClick={() => { const next = detailOrdersPage + 1; if (next > detailOrdersTotalPages) return; loadOrdersPageForCustomer(next); }}
                      className="rounded-full border border-slate-300 bg-white px-3 py-1.5 text-[11px] font-medium hover:bg-slate-50 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-900"
                    >
                      Next
                    </button>
          ...
          {activeTab === "addresses" && (
            <div className="space-y-4 text-sm">
              {detailAddresses.length === 0 ? (
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  {detailAddresses.map((addr) => (
                    <div key={addr.id} className="rounded-2xl border border-slate-200 bg-white p-4 text-sm shadow-sm dark:border-slate-700 dark:bg-slate-900">
                      <div className="mb-2 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex flex-wrap gap-1 text-[10px]">
                          {addr.is_default_shipping && <span className="rounded-full bg-sky-50 px-2 py-0.5 font-medium text-sky-700 dark:bg-sky-900/40 dark:text-sky-200">Default shipping</span>}
                          {addr.is_default_billing && <span className="rounded-full bg-emerald-50 px-2 py-0.5 font-medium text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-200">Default billing</span>}
                        </div>
                      </div>
              ...
              <p className="flex items-center gap-1 text-[11px] text-slate-500 dark:text-slate-400">
                <AlertCircle size={11} />
                Address editing / creation for admin can be wired later to the customer-addresses routes once you finalize the admin endpoints.
              </p>
            </div>
          )}
            ...
            <div className="space-y-6 text-sm">
              <div className="grid gap-4 md:grid-cols-4">
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/40">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">LTV (Lifetime Value)</span>
                    <CreditCard size={14} className="text-slate-400" />
                  </div>
                  <div className="mt-2 text-lg font-semibold">{formatMoney(analytics.ltv)}</div>
                  <div className="mt-1 text-[11px] text-slate-500">Sum of all order grand totals.</div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/40">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">AOV (Average Order Value)</span>
                    <BarChart2 size={14} className="text-slate-400" />
                  </div>
                  <div className="mt-2 text-lg font-semibold">{analytics.totalOrders > 0 ? formatMoney(analytics.aov) : "—"}</div>
                  <div className="mt-1 text-[11px] text-slate-500">LTV / total orders.</div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/40">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="mt-2 text-lg font-semibold">{analytics.totalOrders}</div>
                  <div className="mt-1 text-[11px] text-slate-500">All orders (any status).</div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-900/40">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">Total Items Purchased</span>
                    <Activity size={14} className="text-slate-400" />
                  </div>
                  <div className="mt-2 text-lg font-semibold">{analytics.totalItems}</div>
                  <div className="mt-1 text-[11px] text-slate-500">Sum of items_count across orders.</div>
                </div>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">RFM Scores</span>
                    <Activity size={14} className="text-slate-400" />
                  </div>
                  <div className="mt-3 flex items-center gap-2">
                    <span className="inline-flex flex-col rounded-xl bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-800 dark:bg-slate-800 dark:text-slate-50">
                      <span className="text-[10px] uppercase text-slate-500">R</span>
                  ...
                  <div className="mt-2 text-[11px] text-slate-500">
                    Segment: <span className="font-medium text-slate-700 dark:text-slate-200">{rfmSegmentLabel}</span>
                  </div>
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">First & Last Order</span>
                    <CalendarClock size={14} className="text-slate-400" />
                  </div>
                  <div className="mt-3 space-y-1 text-[13px] text-slate-700 dark:text-slate-200">
                    <div>First order: <span className="font-semibold">{analytics.firstOrderDate ? formatDateTime(analytics.firstOrderDate) : "—"}</span></div>
                    <div>Last order: <span className="font-semibold">{analytics.lastOrderDate ? formatDateTime(analytics.lastOrderDate) : "—"}</span></div>
                  </div>
                  <div className="mt-2 text-[11px] text-slate-500">Helps understand customer lifecycle and recency.</div>
                </div>
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                  <div className="text-xs font-medium text-slate-500">Quick interpretation</div>
                  <div className="mt-2 text-[11px] text-slate-500">Use these metrics to decide:</div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">Revenue over time</span>
                  </div>
                  {analytics.ordersByMonth.length === 0 ? (
                    <div className="py-6 text-center text-xs text-slate-500">Not enough order history to show chart.</div>
                  ) : (
                    <div className="h-52">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
                  <div className="mb-2 flex items-center justify-between">
                    <span className="text-xs font-medium text-slate-500">Items purchased over time</span>
                  </div>
                  {analytics.ordersByMonth.length === 0 ? (
                    <div className="py-6 text-center text-xs text-slate-500">Not enough order history to show chart.</div>
                  ) : (
